...
export function preserveUri(text: string): string {
  return text.replace(new RegExp(patternURI, "g"), function(matched) {
    uri.push(matched);
    return preservation("uri");
  });
}

// bringing back URIs
export function restoreUri(text: string): string {
  return text.replace(
    new RegExp(`[ ]?${preservation("uri").trim()}[ ]?`, "g"),
    () => uri.shift() as string,
  );
}
